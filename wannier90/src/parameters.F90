...
module w90_parameters
  ...
  !! Constrained centres
  logical, public, save :: slab_appro,slab_depend
  integer,public,save :: orbital_number
  ! added by glp
...
!  logical,           public, save :: sigma_abc_onlyorb
  ...
  integer, public, save :: shc_alpha_slab
...
! Module  s p i n
  ...
  real(kind=dp), public, save :: cell_volume,cell_volume_slab
...
! $![ysl-e]
  ...
  integer, public :: nslab
  !! number of layers in slab model
  integer,public :: Ndim
  !! number of dimension of operator in slab model
  integer,public :: ijmax
  !! this is the farest layer considered  in TB model  
contains
  ...
  subroutine param_read()
    ...
    use w90_utility, only: utility_recip_lattice_slab,utility_recip_lattice, utility_metric
...
!    call param_get_keyword('slice_plot_format',found,c_value=slice_plot_format)
    ...
    if (berry .and. index(berry_task, 'ahc') == 0 .and. index(berry_task, 'morb') == 0 &
        ...
        .and. index(berry_task, 'shc') == 0 .and. index(berry_task, 'kdotp') == 0 .and. index(berry_task,'shc_slab')==0) call io_error &
...
!    call param_get_keyword('smear_temp',found,r_value=smear_temp)
    ...
    shc_alpha_slab = 1
    call param_get_keyword('shc_alpha_slab', found, i_value=shc_alpha)
    if (found .and. (shc_alpha < 1 .or. shc_alpha > 2)) call io_error &
      ('Error:  shc_alpha_slab must be 1 or 2 ')
...
!       call io_error('Error: transport_mode.eq.lcr not compatible with tran_read_ht.eq.false')
    ...
    if (eig_found) then
     ...
     nslab=1 
     call param_get_keyword('nslab',found, i_value=nslab)
     if (nslab < 1) call io_error('Error: nslab is at least one')    
     Ndim=nslab*num_wann 
     ijmax=10 

     orbital_number=1
     call param_get_keyword('orbital_number',found, i_value=orbital_number)
     if (nslab < 1) call io_error('Error: orbital_number is at least one')    
   

     slab_appro=.false.
     call param_get_keyword('slab_appro', found, l_value=slab_appro)

     slab_depend=.false.
     call param_get_keyword('slab_depend', found, l_value=slab_depend)
