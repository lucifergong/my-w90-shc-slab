...
!===========================================================
  ...
  real(kind=dp), public, save, allocatable :: wannier_centres_translated_my(:,:)
...
114 call io_error &
  ...
  subroutine fourier_q_to_R(op_q, op_R)
    ...
    use w90_parameters, only: num_wann,num_kpts, kpt_latt,translate_home_cell
    ...
    integer          :: ierr,ir, ik,i,j,n_shift_vec,ii
    real(kind=dp)    :: rdotq
...
!  allocate (wannier_centres_translated_my(3, num_wann), stat=ierr)
!  if (ierr /= 0) call io_error('Error allocatingwannier_centres_translate    d in hamiltonian_setup')
!  wannier_centres_translated_my = 0.0_dp
...
!    op_R = op_R/real(num_kpts, dp)
 ...
 if (translate_home_cell) then 
     ...
     if(lexist)then
        ...
        open(110,file='wann_shift_vec.dat')
        read(110,*)n_shift_vec
        if(n_shift_vec.ne.num_wann)stop'wrong number of shift vectors!stop'
        do i=1,num_wann
           read(110,*)ii,shift_vec(:,i)
        end do
        close(110)
  !   else
  !      call internal_translate_centres()
     ...
     do ir = 1, nrpts
       do ik = 1, num_kpts
         do i=1,num_wann
           do j=1,num_wann
              irvec_tmp=irvec(:,ir)+shift_vec(:,i)-shift_vec(:,j)
              rdotq = twopi*dot_product(kpt_latt(:, ik),irvec_tmp(:))
              phase_fac = exp(-cmplx_i*rdotq)
              op_R(:, :, ir) = op_R(:, :, ir) + phase_fac*op_q(:, :, ik)
           enddo
         enddo
...
!~      end if
      ...
      wannier_centres_translated_my = r_home
