...
module w90_wan_ham
  ...
  public :: wham_get_D_h,wham_get_D_h_slab,wham_get_eig_deleig,wham_get_eig_deleig_slab,wham_get_eig_deleig_TB_conv, wham_get_D_h_P_value
  public :: wham_get_occ_mat_list, wham_get_eig_UU_HH_JJlist
  public :: wham_get_eig_UU_HH_AA_sc, wham_get_eig_UU_HH_AA_sc_TB_conv
  public :: wham_get_eig_deleig_temp,wham_get_D_h_temp
contains
 ...
 subroutine wham_get_D_h_temp(delHH, UU, eig, D_h)
    !=========================================!
    !                                         !
    ...
    use w90_parameters, only:kubo_smr_fixed_en_width,num_wann,Ndim
    use w90_utility, only: utility_rotate
    ...
    integer                       :: n, m, i
    real(kind=dp)                 :: eta
    allocate (delHH_bar_i(Ndim, Ndim))
    eta=kubo_smr_fixed_en_width 
    D_h = cmplx_0
    do i = 1, 3
      delHH_bar_i(:, :) = utility_rotate(delHH(:, :, i), UU,Ndim)
      do m = 1, Ndim
        do n = 1, Ndim
          if (n == m .or. abs(eig(m) - eig(n)) < 1.0e-5_dp) cycle
        ! if (n == m .or. abs(eig(m) - eig(n)) < 5.0e-3_dp) cycle
          !  D_h(n, m, i) = 1/(eig(m) - eig(n))
             D_h(n, m, i) = delHH_bar_i(n, m)/(eig(m) - eig(n))
          !  D_h(n, m, i) = delHH_bar_i(n, m)
        end do
      end do
    enddo

  end subroutine wham_get_D_h_temp
  ...
  subroutine wham_get_D_h_slab(delHH, UU, eig, D_h)
    !=========================================!
    !                                         !
    !! Compute D^H_a=UU^dag.del_a UU (a=x,y,z)
    !! using Eq.(24) of WYSV06
    ...
    use w90_constants, only: dp, cmplx_0
    use w90_parameters, only: num_wann,nslab,Ndim
    use w90_utility, only: utility_rotate
  ...
  end subroutine wham_get_D_h_slab
...
!  subroutine wham_get_occ_mat_list(eig,UU,f_list,g_list)
  ...
  subroutine wham_get_deleig_a_slab(deleig_a, eig, delHH_a, UU)
    !==========================!
    !                          !
    ...
    use w90_utility, only: utility_diagonalize, utility_rotate, &
      utility_rotate_diag
    use w90_parameters, only: num_wann,use_degen_pert,degen_thr,nslab,Ndim

    ! Arguments
    !
    real(kind=dp), intent(out) :: deleig_a(Ndim)
    ...
    Ndim=num_wann*nslab
    allocate (delHH_bar_a(Ndim, Ndim))
    allocate (U_deg(Ndim, Ndim))

    if (use_degen_pert) then

      delHH_bar_a = utility_rotate(delHH_a, UU,Ndim)
      ...
      !
      deleig_a(:) = real(utility_rotate_diag(delHH_a(:, :), UU,Ndim), dp)

    end if

  end subroutine wham_get_deleig_a_slab
  ...
  subroutine wham_get_deleig_a_temp(deleig_a, eig, delHH_a, UU)
    !==========================!
    !                          !
    !! Band derivatives dE/dk_a
    !                          !
    ...
    use w90_utility, only: utility_diagonalize, utility_rotate, &
      utility_rotate_diag
    use w90_parameters, only: num_wann,Ndim,nslab,use_degen_pert, degen_thr
  ...
  end subroutine wham_get_deleig_a_temp
  ...
  subroutine wham_get_eig_deleig_slab(kpt, eig,del_eig,HH,delHH,UU)
!  subroutine wham_get_eig_deleig_slab(kpt, eig, del_eig, HH, delHH, UU) 
    ...
    use w90_postw90_common, only: pw90common_fourier_R_to_k_slab
    use w90_utility, only: utility_diagonalize,utility_eigensystem_c
    real(kind=dp), dimension(2), intent(in)         :: kpt
    !! the three coordinates of the k point vector (in relative coordinates)
    real(kind=dp), intent(out)                      :: eig(Ndim)
    !! the calculated eigenvalues at kpt
    real(kind=dp), intent(out)                      :: del_eig(Ndim,2)
    ...
    call pw90common_fourier_R_to_k_slab(kpt, HH_R, HH, 0)
    call utility_diagonalize(HH, Ndim, eig, UU)
    call pw90common_fourier_R_to_k_slab(kpt, HH_R, delHH(:, :, 1), 1)
    call pw90common_fourier_R_to_k_slab(kpt, HH_R, delHH(:, :, 2), 2)
    call wham_get_deleig_a(del_eig(:, 1), eig, delHH(:, :, 1), UU)
    call wham_get_deleig_a(del_eig(:, 2), eig, delHH(:, :, 2), UU)
  end subroutine wham_get_eig_deleig_slab
  ...
  subroutine wham_get_eig_deleig_temp(kpt, eig, del_eig, HH, delHH, UU)
    !! Given a k point, this function returns eigenvalues E and
    !! derivatives of the eigenvalues dE/dk_a, using wham_get_deleig_a
    !
    use w90_constants ,only : cmplx_0
    use w90_parameters, only: num_wann,Ndim,slab_appro
    use w90_get_oper, only: HH_R, get_HH_R
    use w90_postw90_common, only:pw90common_fourier_R_to_k,pw90common_fourier_r_to_k_temp
    use w90_utility, only: utility_diagonalize,utility_eigensystem_c
    ...
    real(kind=dp), intent(out)                      :: eig(Ndim)
    !! the calculated eigenvalues at kpt
    real(kind=dp), intent(out)                      :: del_eig(Ndim, 3)
    !! the calculated derivatives of the eigenvalues at kpt [first component: band; second component: 1,2,3
    !! for the derivatives along the three k directions]
    !complex(kind=dp), dimension(:, :), intent(out)   :: HH
    complex(kind=dp), dimension(Ndim,Ndim), intent(out)   :: HH
    !! the Hamiltonian matrix at kpt
    complex(kind=dp), dimension(:, :, :), intent(out) :: delHH
    !! the delHH matrix (derivative of H) at kpt
    complex(kind=dp), dimension(:, :), intent(out)   :: UU
    ...
    ! Further calls should return very fast.
    integer   ::  i,j
    complex(kind=dp) :: n(Ndim),wave(Ndim),c
    
    call get_HH_R
    call pw90common_fourier_R_to_k_temp(kpt, HH_R, HH, 0)
   do i=1,Ndim
    do j=1,Ndim
      if  ( abs(HH(i,j)-conjg(HH(j,i))) > 1e-3 ) then 
       write(*,*) 'hermitian is not preserved in the hamitonian '
       endif
   ...
   UU=cmplx_0
   UU=HH
   call utility_eigensystem_c( 'V', 'U', Ndim ,UU,eig)
  
  ! call utility_diagonalize(HH, Ndim, eig, UU)
   
  !here i will use a method to avoid the random phase
  ...
  enddo
   ...
   c=0
   do j=1,Ndim
   c=c+conjg(wave(j))*n(j)
   enddo 
   wave=wave*c 
   wave=wave/sqrt(c*conjg(c))
   UU(1:Ndim,i)=wave
   enddo 
  ...
  !this is the end
   ...
   call pw90common_fourier_R_to_k_temp(kpt, HH_R, delHH(:, :, 3), 3)
     ...
     call wham_get_deleig_a_temp(del_eig(:, 1), eig, delHH(:, :, 1), UU)
     call wham_get_deleig_a_temp(del_eig(:, 2), eig, delHH(:, :, 2), UU)
     call wham_get_deleig_a_temp(del_eig(:, 3), eig, delHH(:, :, 3), UU)
     if ( slab_appro ) then 
          delHH(:,:,3) = cmplx_0
         del_eig(:, 3) = 0d0
     endif

   end subroutine wham_get_eig_deleig_temp
