...
contains
  ...
  subroutine k_slice
    ...
    use w90_comms
    use w90_constants, only: dp, twopi, eps8,cmplx_0,cmplx_i
    use w90_io, only: io_error, io_file_unit, seedname, &
      io_time, io_stopwatch, stdout
    use w90_utility, only: utility_diagonalize,utility_recip_lattice,utility_recip_lattice_slab
    use w90_postw90_common, only: pw90common_fourier_R_to_k_vec_temp,pw90common_fourier_R_to_k_temp,pw90common_fourier_R_to_k
    use w90_parameters, only: Ndim,slab_appro,real_lattice,num_wann, kslice, kslice_task, kslice_2dkmesh, &
    ...
    use w90_get_oper, only: get_HH_R, HH_R,  get_BB_R, get_CC_R, & 
       get_SS_R, get_SHC_R ,AA_R ,get_AA_R
    use w90_wan_ham, only:wham_get_D_h_temp,wham_get_eig_deleig_temp, wham_get_eig_deleig
    use w90_spin, only: spin_get_nk
    use w90_berry, only: berry_get_imf_klist,berry_get_shc_klist_temp_way2,berry_get_imfgh_klist, berry_get_shc_klist
    ...
    real(kind=dp)     :: shc_k_band(Ndim),slab_cell_volume,slab_recip_lattice(3,3),avec_2d(3, 3), avec_3d(3, 3), bvec(3, 3), yvec(3), zvec(3), &
    ...
    !
    complex(kind=dp),allocatable ::AA1(:,:,:),AA2(:,:,:),D_h(:,:,:) 
    real(kind=dp) :: kpt_2(3)
    !ADD by glp
  ...
  call utility_recip_lattice_slab(real_lattice,slab_recip_lattice,slab_cell_volume)
    ...
    if (slab_appro) then
    bvec(1, :) = matmul(kslice_b1(:), slab_recip_lattice(:,:) )
    bvec(2, :) = matmul(kslice_b2(:), slab_recip_lattice(:,:) )
    endif
    ...
    if (plot_shc) then 
      ...
      !allocate (AA1(num_wann,num_wann,3),AA2(num_wann,num_wann,3))
    endif
    !add by glp
    ...
    do iloc = 1, my_nkpts
      ...
      if (slab_appro) then
 kcorner_cart(:)= kslice_corner(1)*bvec(1,:)+kslice_corner(2)*bvec(2,:)
      endif
...
!                   Delta_E=Delta_E*sqrt(2.0_dp) ! optimize this factor
      ...
      ! call pw90common_fourier_R_to_k_vec_temp(kpt, AA_R, OO_true=AA1)
      ! AA1 = AA1 + cmplx_i*D_h 
      ...
      ! kpt_2(2)=-kpt(2)
      ! kpt_2(3)=kpt(3) 
      ! call wham_get_eig_deleig_temp(kpt_2, eig, del_eig, HH, delHH, UU)
      ! call wham_get_D_h_temp(delHH, UU, eig, D_h)
      ! call pw90common_fourier_R_to_k_vec_temp(kpt_2, AA_R, OO_true=AA2)
      ! AA2 = AA2 + cmplx_i*D_h
     ...
     !  write(*,*)AA1(:,:,2)  
        ...
        ! do i=1,46
        ! my_zdata(1, iloc) = my_zdata(1, iloc)+shc_k_band(i)
        ! enddo
         
        ! my_zdata(1, iloc) = shc_k_band(29)
         
        !my_zdata(1, iloc) = shc_k_band(24)
        
        my_zdata(1, iloc) = shc_k_band(29)
        !my_zdata(1, iloc) = shc_k_band(47)+shc_k_band(48) !conduect     
         
         !my_zdata(1, iloc) = shc_k_band(49)+shc_k_band(50) !valence
         !my_zdata(1, iloc) = eig(47) 
         !my_zdata(1, iloc) = eig(29) 
         !my_zdata(1, iloc) = 0d0 
         !my_zdata(1, iloc) = eig(30) 
         !my_zdata(1, iloc) =0d0
      ...
      !kpt_2(2)=-kpt(2)
      !kpt_2(3)=kpt(3) 
       !call berry_get_shc_klist_temp_way2(kpt_2,shc_k_band=shc_k_band) 
       
      !call wham_get_eig_deleig_temp(kpt_2, eig, del_eig, HH, delHH, UU)
       
      !my_zdata(1, iloc) = my_zdata(1, iloc)-eig(29)


       !my_zdata(1, iloc) = my_zdata(1, iloc)-shc_k_band(29)
      !write(*,*)my_zdata(1, iloc)
