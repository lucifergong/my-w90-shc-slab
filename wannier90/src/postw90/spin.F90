...
! =========================================================================
  ...
  subroutine spin_get_nk_temp(kpt, spn_nk)
    !=============================================================!
    !                                                             !
  ...
  end subroutine spin_get_nk_temp
...
! =========================================================================
  ...
  subroutine spin_get_nk(kpt, spn_nk)
    ...
    real(kind=dp), intent(in)  :: kpt(3)
    real(kind=dp), intent(out) ::spn_nk(num_wann)
    real(kind=dp) :: spn_xk(num_wann),spn_yk(num_wann),spn_zk(num_wann)

    ! Physics
    !
    complex(kind=dp), allocatable :: HH(:, :)
    complex(kind=dp), allocatable :: UU(:, :)
    complex(kind=dp), allocatable :: SS(:, :, :),SS_n(:, :),SS_x(:,:),SS_y(:,:),SS_z(:, :)
    ...
    allocate (UU(num_wann, num_wann))
    allocate (SS(num_wann, num_wann, 3))
    allocate (SS_n(num_wann, num_wann),SS_x(num_wann, num_wann),SS_y(num_wann,num_wann),SS_z(num_wann, num_wann))
    ...
    SS_y(:, :) = SS(:, :, 2)
    SS_z(:, :) = SS(:, :, 3)
    !spn_xk(:) = real(utility_rotate_diag(SS_x, UU, num_wann), dp)
    !spn_yk(:) = real(utility_rotate_diag(SS_y, UU, num_wann), dp)
    !spn_zk(:) = real(utility_rotate_diag(SS_z, UU, num_wann), dp)
   

    do n=1,num_wann
    !spn_nk(n)=spn_xk(n)**2+spn_yk(n)**2+spn_zk(n)**2
    spn_nk(n)=SS_x(n,n)**2+SS_y(n,n)**2+SS_z(n,n)**2
    enddo 
