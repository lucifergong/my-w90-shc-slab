...
contains
  ...
  subroutine k_path
    ...
    use w90_postw90_common, only: pw90common_fourier_R_to_k_temp,&
    pw90common_fourier_R_to_k
    use w90_parameters, only: Ndim,num_wann, kpath_task, &
    ...
    use w90_berry, only: berry_get_imf_klist, berry_get_imfgh_klist, &
      berry_get_shc_klist,berry_get_shc_klist_temp_way2,projectcomponent
    use w90_constants, only: bohr

    integer, dimension(0:num_nodes - 1) :: counts, displs

    integer           :: i, j, n,k, num_paths, num_spts, loop_path, loop_kpt, &
  ...
  !  real(kind=dp)     :: shc_k_band(num_wann), shc_k_fermi(nfermi)
    real(kind=dp)     :: component(Ndim),shc_k_band(Ndim),shc_k_fermi(nfermi)
   ...
   ! allocate (UU(num_wann, num_wann))  
     ...
     allocate (my_eig(Ndim, my_num_pts))
   ! allocate (my_eig(num_wann, my_num_pts))

      if (kpath_bands_colour /= 'none') allocate (my_color(Ndim, my_num_pts))
    ...
    do loop_kpt = 1, my_num_pts
      ...
      ! call pw90common_fourier_R_to_k(kpt, HH_R, HH, 0)
        call utility_diagonalize(HH, Ndim, my_eig(:, loop_kpt), UU)
      ...
      !
        ...
        if (kpath_bands_colour == 'spin') then
          ...
          !!!add by glp 
!      do n=1,(Ndim/2)
!my_color(2*n,loop_kpt)=(spn_k(2*n)+spn_k(2*n-1))/2
!my_color(2*n-1,loop_kpt)=(spn_k(2*n)+spn_k(2*n-1))/2
!     enddo
...
!    add some code to mix the shc of the degenerated band
    k=Ndim/2
     do n=1,k   
    my_color(2*n,loop_kpt)=(shc_k_band(2*n)+shc_k_band(2*n-1))/2
  my_color(2*n-1,loop_kpt)=(shc_k_band(2*n)+shc_k_band(2*n-1))/2
     enddo
! end the code


!      call projectcomponent(kpt,component=component)   
!      my_color(:, loop_kpt) = component
       end if
      end if
    ...
    if (plot_bands) then
      allocate (eig(Ndim, total_pts))
      call comms_gatherv(my_eig, Ndim*my_num_pts, &
                         eig, Ndim*counts, Ndim*displs)
      if (kpath_bands_colour /= 'none') then
        allocate (color(Ndim, total_pts))
        call comms_gatherv(my_color,Ndim*my_num_pts, &
                           color, Ndim*counts, Ndim*displs)
    ...
    if (on_root) then
      ...
      if (plot_bands .and. plot_shc) then
        ...
        write (pyunit, '(a)') "pl.xlim([0,max(x)])"
        write (pyunit, '(a)') "pl.ylim([-0.5,0.5]) # Adjust this range as needed"
