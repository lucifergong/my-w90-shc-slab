...
module w90_berry
  ...
  public :: projectcomponent,berry_main, berry_get_shc_klist_temp_way2,berry_get_imf_klist, berry_get_imfgh_klist, berry_get_sc_klist, &
            berry_get_shc_klist, berry_get_kdotp,berry_get_shc_klist_slab!, berry_alpha_S, berry_alpha_beta_S, berry_beta_S
...
contains
  ...
  subroutine berry_main
    ...
    use w90_postw90_common, only: nrpts, irvec, num_int_kpts_on_node, int_kpts, &
      weight,pw90common_ham_slab,pw90common_fourier_r_to_k_temp,pw90common_fourier_R_to_k_slab,pw90common_fourier_R_to_k,pw90common_OO_qlayer2qlayer,pw90common_qlayer2qlayer_supperlattice,pw90common_fourier_R_to_k_new,pw90common_fourier_R_to_k_vec_temp,pw90common_fourier_r_to_k_vec
    use w90_parameters, only: slab_depend,wannier_centres,shc_alpha,timing_level, iprint, num_wann, berry_kmesh, &
      berry_curv_adpt_kmesh, &
      berry_curv_adpt_kmesh_thresh, &
      wanint_kpoint_file, cell_volume,cell_volume_slab,transl_inv, &
      ...
      kdotp_kpoint, kdotp_num_bands, kdotp_bands,Ndim,nslab,ijmax,param_dist
    use w90_get_oper, only: get_SHC_R,get_HH_R, get_AA_R, get_BB_R, get_CC_R, &
      get_SS_R, get_SHC_R, get_SAA_R, get_SBB_R, HH_R,AA_R,SH_R
   !!! HH_R is added by glp when test 
   use w90_utility, only: utility_diagonalize
   !! this is also added by glp 
   use w90_wan_ham, only: wham_get_eig_deleig_temp,wham_get_eig_deleig,wham_get_eig_deleig_slab,wham_get_D_h_slab 
   !! this is also added by glp 
   
   real(kind=dp), allocatable    :: adkpt(:, :)
   real(kind=dp), allocatable    :: adkptslab(:,:)
   !added by 龚令朴
    ...
    real(kind=dp), allocatable  :: shc_fermi(:),shc_k_fermi(:),shc_fermi_slab(:,:),shc_k_fermi_slab(:,:)
    complex(kind=dp), allocatable :: shc_freq(:), shc_k_freq(:),shc_freq_slab(:),shc_k_freq_slab(:)
    ! for fermi energy scan, adaptive kmesh
    real(kind=dp), allocatable    :: shc_k_fermi_dummy(:),shc_k_fermi_dummy_slab(:)

    real(kind=dp)     :: kweight, kweight_adpt,kweight_slab, kweight_adpt_slab,kpt(3), kpt_ad(3), &
    db1, db2, db3,dbb3,fac, freq, rdum, vdum(3),kptslab(2)
    integer           :: kmesh3,n, i, j, k, jk, ikpt, if, ispn, ierr, loop_x, loop_y, loop_z, &
    ...
    logical           :: eval_ahc, eval_morb, eval_kubo, not_scannable, eval_sc, eval_shc, &
                      eval_kdotp,eval_shc_slab
    logical           :: ladpt_kmesh
    logical           :: ladpt(nfermi)
     

    !!! test command by lingpugong
    complex(kind=dp)  :: UU(Ndim,Ndim)
    complex(kind=dp)  :: UU_my(Ndim,Ndim)
    real(kind=dp)  :: eig(Ndim)
    real(kind=dp)  :: eig_my(Ndim)
    complex(kind=dp)  :: HH(Ndim,Ndim) 
    complex(kind=dp)  :: HH_my(Ndim,Ndim)
    real(kind=dp)  :: del_eig(Ndim,2)
    complex(kind=dp)  :: delHH(Ndim,Ndim,3)
    complex(kind=dp)  :: delHH_my(Ndim,Ndim,3) 
    complex(kind=dp)  :: D_h(Ndim,Ndim,2)
    complex(kind=dp)  :: js_k(Ndim,Ndim)
    complex(kind=dp)  :: SH_w(Ndim, Ndim, 3)


!    complex(kind=dp)  :: HH(num_wann,num_wann)
!     complex(kind=dp), dimension(:, :)   :: HH
    complex(kind=dp), dimension(-ijmax:ijmax,num_wann,num_wann) ::HH_su
    complex(kind=dp), dimension(-ijmax:ijmax,Ndim,Ndim) ::HH_supper
    complex(kind=dp), dimension(Ndim,Ndim,3) ::AA
    complex(kind=dp), dimension(Ndim,Ndim,3) ::AA_my
    real,dimension(2,num_wann) ::wannier_center_order
    real ::centerreplace,indxreplace
    real(kind=dp)     :: shc_k_band(Ndim) 
   HH_su=cmplx_0
   HH_supper=cmplx_0
   HH_my=cmplx_0
   HH=cmplx_0
   ...
   UU=cmplx_0
    ...
    if (index(berry_task, 'shc_slab') > 0) eval_shc_slab = .true.
  ...
  !    endif
      if (shc_freq_scan ) then
        ...
        allocate (shc_k_fermi_slab(nfermi,nslab))
        allocate (shc_fermi_slab(nfermi,nslab))

        allocate (shc_fermi(nfermi))
        allocate (shc_k_fermi(nfermi))
        allocate (shc_k_fermi_dummy(nfermi))

        shc_k_fermi_slab=0.0_dp
        shc_fermi_slab=0d0
        shc_fermi = 0.0_dp
        shc_k_fermi = 0.0_dp
    ...
    if (on_root) then
      ...
      if (eval_shc_slab) then
        write (stdout, '(/,3x,a)') '* Spin Hall Conductivity for slab model'
        if (index(shc_method, 'qiao') > 0) then
          write (stdout, '(/,3x,a)') '  Qiao''s SHC (Phys.Rev.B 98.214402)'
        else
          write (stdout, '(/,3x,a)') '  Ryoo''s SHC (Phys.Rev.B 99.235113)'
    ...
    allocate(adkptslab(2,berry_curv_adpt_kmesh**2),stat=ierr)
    if (ierr /= 0) call io_error('Error in allocating adkptslab in berry')

!    allocate (adkpt(3, berry_curv_adpt_kmesh**3), stat=ierr)
!    if (ierr /= 0) call io_error('Error in allocating adkpt in berry')
   
   allocate (adkpt(3, kmesh3*berry_curv_adpt_kmesh**2), stat=ierr)
    if (ierr /= 0) call io_error('Error in allocating adkpt in berry')
   ...
   ikpt = 0
    do i = 0 ,berry_curv_adpt_kmesh-1
       ...
       ikpt = ikpt + 1
    adkptslab(1, ikpt) = db1*((i + 0.5_dp)/berry_curv_adpt_kmesh - 0.5_dp)
    adkptslab(2, ikpt) = db2*((j + 0.5_dp)/berry_curv_adpt_kmesh - 0.5_dp)
       end do
    end do
  ...
  !      do k = 0, berry_curv_adpt_kmesh - 1
     ...
     !     adkpt(3, ikpt) = db3*((k + 0.5_dp)/berry_curv_adpt_kmesh - 0.5_dp)
          adkpt(3, ikpt) = dbb3*((k + 0.5_dp)/kmesh3 - 0.5_dp)
...
!          call berry_get_shc_klist_temp(kpt, shc_k_fermi=shc_k_fermi)
     call berry_get_shc_klist_temp_way2(kpt, shc_k_fermi=shc_k_fermi)
...
!          call berry_get_shc_klist_temp(kpt, shc_k_freq=shc_k_freq)
!          call berry_get_shc_klist(kpt, shc_k_freq=shc_k_freq)
         call berry_get_shc_klist_temp_way2(kpt, shc_k_freq=shc_k_freq) 
...
!  if (on_root) then      
!   wannier_center_order(1,1)=wannier_centres(3,1)
!   wannier_center_order(2,1)=1
!   do i=2,num_wann
!       wannier_center_order(1,i)=wannier_centres(3,i)
!       wannier_center_order(2,i)=i
!   do j=i-1,1,-1
!   if (wannier_center_order(1,j+1) < wannier_center_order(1,j)) then
!               centerreplace=wannier_center_order(1,j)
!               wannier_center_order(1,j)=wannier_center_order(1,j+1)
!               wannier_center_order(1,j+1)=centerreplace
!  
!               indxreplace=wannier_center_order(2,j)
!               wannier_center_order(2,j)=wannier_center_order(2,j+1)
!               wannier_center_order(2,j+1)=indxreplace
!   else 
!           exit
...
!   enddo
!             write(*,*)wannier_center_order(2,:),wannier_center_order(1,:)
!  endif       
...
!          if (.not. shc_freq_scan) then
!     call berry_get_shc_klist_slab(kptslab, shc_k_fermi=shc_k_fermi_slab)
      


  ! finish  call  pw90common_OO_qlayer2qlayer(kptslab,HH_R, HH,0)
  ! finish  call  pw90common_fourier_R_to_k_slab(kptslab,HH_R,HH,0)
  ! finish  call  utility_diagonalize(HH,Ndim,eig,UU)
  ! finish  call  pw90common_fourier_R_to_k_slab(kptslab,HH_R,HH,1) 
  ! finish  call  wham_get_eig_deleig_slab(kptslab,eig,del_eig,HH,delHH,UU)
  ! finish  call  wham_get_D_h_slab(delHH,UU,eig,D_h)  



!    call  pw90common_OO_qlayer2qlayer(kptslab,HH_R,HH_su,0)
!    call  pw90common_qlayer2qlayer_supperlattice(kptslab,HH_R,HH_supper,0)

! finish   call  pw90common_fourier_R_to_k(kpt,HH_R,HH,0) 
! finish   call  pw90common_fourier_R_to_k_temp(kpt, HH_R, HH_my,3)
! finish   call  pw90common_fourier_R_to_k_new(kpt,HH_R,OO_dz=HH)

!finish    call pw90common_fourier_R_to_k_vec_temp(kpt,AA_R,AA_my)
!finish    call pw90common_fourier_R_to_k_vec(kpt,AA_R,AA)

!multyslab
!finish   call  pw90common_fourier_R_to_k_temp(kpt, HH_R,HH_my,3)
!finish   call  pw90common_fourier_R_to_k_vec_temp(kpt,AA_R,AA_my)

!call pw90common_fourier_R_to_k_vec_temp(kpt,SH_R,SH_w)

!   call pw90common_fourier_R_to_k_temp(kpt,SH_R(:,:,:,1),SH_w(:,:,1),0)
!   call pw90common_fourier_R_to_k_temp(kpt,SH_R(:,:,:,2),SH_w(:,:,2),0)
!   call pw90common_fourier_R_to_k_temp(kpt,SH_R(:,:,:,3),SH_w(:,:,3),0)
...
!        call  pw90common_fourier_R_to_k_temp(kpt, HH_R, HH_my,0)
! finish   call  pw90common_fourier_R_to_k_new(kpt,HH_R,OO_dz=HH)


!   call pw90common_fourier_R_to_k_vec_temp(kpt,AA_R,AA_my)
!   call pw90common_fourier_R_to_k_vec(kpt,AA_R,AA)
!HH=HH-HH_my
!do i=1,Ndim
!do j=1,Ndim
!do k=1,3
!if (abs(HH(i,j)) < 1d-4 ) then 
!     HH(i,j)=0
...
!         kpt(2)=0.0
!         kpt(3)=0.0
!    call berry_get_shc_klist_temp_way2(kpt,shc_k_band=shc_k_band)
!    open(unit=3,file='shc_k_band')
!
!    write(3,*)'THIS IS shc_k_band'
!    write(3,*)shc_k_band 
!      
!     endif
      ...
      kweight = db1*db2*dbb3
      kweight_adpt = kweight/(berry_curv_adpt_kmesh**2*kmesh3)
  ...
  do loop_xyz=my_node_id,berry_kmesh(1)*berry_kmesh(2)*kmesh3-1,num_nodes
        loop_x = loop_xyz/(berry_kmesh(2)*kmesh3)
        loop_y = (loop_xyz - loop_x*(berry_kmesh(2) &
                                     *kmesh3))/kmesh3
        loop_z = loop_xyz - loop_x*(berry_kmesh(2)*kmesh3) &
                 - loop_y*kmesh3
        kpt(1) = loop_x*db1
        kpt(2) = loop_y*db2
        kpt(3) = loop_z*dbb3

!  call  pw90common_OO_qlayer2qlayer(kpt(1:2),HH_R,HH_su,0)
!  call  pw90common_qlayer2qlayer_supperlattice(kpt(1:2),HH_R,HH_supper,0)
...
!        kpt(3) = loop_z*db3
        ...
        if (eval_shc) then
          ...
          !   berry_get_shc_klist -> wham_get_eig_deleig ->
          !   pw90common_fourier_R_to_k -> ws_translate_dist
          call berry_print_progress(loop_xyz,my_node_id,berry_kmesh(1)*berry_kmesh(2)*kmesh3-1,num_nodes)
          if (.not. shc_freq_scan) then
!           call berry_get_shc_klist(kpt, shc_k_fermi=shc_k_fermi)
!          call berry_get_shc_klist_temp(kpt, shc_k_fermi=shc_k_fermi)

      if (slab_depend ) then 
      call  berry_get_shc_klist_temp_way3(kpt, shc_k_fermi_slab=shc_k_fermi_slab)
      else 
      call  berry_get_shc_klist_temp_way2(kpt, shc_k_fermi=shc_k_fermi)
      endif
      ...
      !do i=1,nfermi    
      !do k=1,nslab-1
      !   write(*,*)shc_fermi_slab(i,k)-shc_fermi_slab(i,k+1)
      !enddo !k
      !enddo
           ...
           !check whether needs to tigger adpt kmesh or not.
            ...
            if (ladpt_kmesh) then
             ! do loop_adpt = 1, berry_curv_adpt_kmesh**3
                 ...
                 do loop_adpt = 1, kmesh3*berry_curv_adpt_kmesh**2
                ! modified by glp
     !
                !Using shc_k here would corrupt values for other
                !kpt, hence dummy. Only if-th element is used.
...
!                                         shc_k_fermi=shc_k_fermi_dummy)
     ...
     call berry_get_shc_klist_temp_way2(kpt(:) + adkpt(:, loop_adpt), &
              ...
              if (slab_depend) then      
              shc_fermi_slab = shc_fermi_slab + kweight*shc_k_fermi_slab
...
!             call berry_get_shc_klist(kpt, shc_k_freq=shc_k_freq)
    ...
    if (eval_shc) then
      if (shc_freq_scan) then
         ...
         if (slab_depend) then  
            call comms_reduce(shc_fermi_slab(1,1),nfermi*nslab,'SUM')
         else        
    ...
    if (on_root) then
      ...
      ! -----------------------!
      !
      if ((eval_shc) .and. (slab_depend ==.false.)) then
        !
     ...
     !  enddo
       ...
       ! fac = 1.0e8_dp*elem_charge_SI**2/(hbar_SI*cell_volume)
        if (shc_freq_scan) then
          ...
          shc_fermi_slab = shc_fermi_slab*fac
        !这里记得改回来
        endif
        !
        ...
        if (.not. shc_freq_scan) then
          if (slab_depend) then
          file_name = trim(seedname)//'-shc-slab-depend-fermiscan'//'.dat'
          else
          file_name = trim(seedname)//'-shc-fermiscan'//'.dat'
          endif
        else
        ...
        do i=1,nslab
           write(ctemp,'(i2)')i
           file_unit = io_file_unit()
           open (file_unit, FILE=trim(adjustl( cTemp ))//file_name, STATUS='UNKNOWN', FORM='FORMATTED')
           if (.not. shc_freq_scan) then
              write (file_unit, '(a,3x,a,3x,a)') &
              '#No.', 'Fermi energy(eV)', 'SHC((hbar/e)*S/cm)'
              do n = 1, nfermi
              write (file_unit, '(I4,1x,F12.6,1x,E17.8)') &
               n,fermi_energy_list(n), shc_fermi_slab(n,i)
              
!              if(i<nslab-1) then 
!               write(*,*) shc_fermi_slab(n,i)-shc_fermi_slab(n,i+1)
!              endif 
...
!        fac = 1.0e8_dp*elem_charge_SI**2/(hbar_SI*cell_volume)/2.0_dp
!        if (shc_freq_scan) then
!          shc_freq_slab = shc_freq_slab*fac
!        else
!          shc_fermi_slab =shc_fermi_slab*fac
!        endif
        !
...
!          '----------------------------------------------------------'
!        write (stdout, '(1x,a)') &
!          'Output data files related to slab Spin Hall conductivity:'
!        write (stdout, '(1x,a)') &
!          '----------------------------------------------------------'
!        !
!        if (.not. shc_freq_scan) then
!          file_name = trim(seedname)//'-shc_slab-fermiscan'//'.dat'
!        else
 !         file_name = trim(seedname)//'-shc_slab-freqscan'//'.dat'
!        endif
!        file_name = trim(file_name)
!        file_unit = io_file_unit()
!        write (stdout, '(/,3x,a)') '* '//file_name
...
!          do n = 1, nfermi
!            write (file_unit, '(I4,1x,F12.6,1x,E17.8)') &
!              n, fermi_energy_list(n), shc_fermi_slab(n)
!          enddo
!        else
!          write (file_unit, '(a,3x,a,3x,a,3x,a)') '#No.', 'Frequency(eV)', &
!            'Re(sigma)((hbar/e)*S/cm)', 'Im(sigma)((hbar/e)*S/cm)'
!          do n = 1, kubo_nfreq
!            write (file_unit, '(I4,1x,F12.6,1x,1x,2(E17.8,1x))') n, &
!              real(kubo_freq_list(n), dp), real(shc_freq_slab(n), dp), aimag(shc_freq_slab(n))
!          enddo
!        endif
!        close (file_unit)
!
...
!                        D_h(:, :, shc_alpha), UU, js_k)
  ...
  !      prod = js_k(n, m)*cmplx_i*rfac*AA(m, n, shc_beta)
        ...
        prod = -cmplx_i*AA(n, m, shc_alpha)*cmplx_i*AA(m, n, shc_beta)
  ...
  subroutine berry_get_shc_klist_temp_way2(kpt, shc_k_fermi, shc_k_freq, shc_k_band)
    !====================================================================!
    !                                                                    !
    ! Contribution from a k-point to the spin Hall conductivity on a list
    ! of Fermi energies or a list of frequencies or a list of energy bands
    ...
    use w90_constants, only: dp, cmplx_0, cmplx_i
    use w90_utility, only: utility_rotate
    use w90_parameters, only: num_wann,nslab,Ndim,kubo_eigval_max, kubo_nfreq, &
      kubo_freq_list, kubo_adpt_smr, kubo_smr_fixed_en_width, &
      kubo_adpt_smr_max, kubo_adpt_smr_fac, berry_kmesh, &
      fermi_energy_list, nfermi, shc_alpha, shc_beta, shc_gamma, &
      shc_bandshift, shc_bandshift_firstband, shc_bandshift_energyshift
use w90_postw90_common, only:pw90common_get_occ_temp,pw90common_get_occ,pw90common_get_occ_slab,pw90common_fourier_R_to_k_vec_slab,pw90common_kmesh_spacing,pw90common_fourier_R_to_k_vec,pw90common_fourier_R_to_k_vec_temp 
    use w90_wan_ham, only: wham_get_D_h_temp,wham_get_D_h,wham_get_eig_deleig,wham_get_eig_deleig_temp
    use w90_get_oper, only: AA_R
    !use w90_comms, only: my_node_id
    !!!
    ...
    real(kind=dp), optional, intent(out) :: shc_k_fermi(nfermi)
    complex(kind=dp), optional, intent(out) :: shc_k_freq(kubo_nfreq)
    real(kind=dp), optional, intent(out) :: shc_k_band(Ndim)
    ...
    complex(kind=dp), allocatable :: D_h(:, :, :)
    complex(kind=dp), allocatable :: AA(:, :, :)
    complex(kind=dp)              :: js_k(Ndim,Ndim),omegamatrix(Ndim,Ndim)
    !real(kind=dp) :: omegamatrix(Ndim,Ndim)


    ! Adaptive smearing
    !
    real(kind=dp)    :: del_eig(Ndim,3), joint_level_spacing, &
                        eta_smr, Delta_k, vdum(3)

    integer          :: occupynum,n, m, i,s, ifreq,k,pro
    real(kind=dp)    :: eig(Ndim)
    real(kind=dp)    :: occ_fermi(Ndim,nfermi), occ_freq(Ndim)
    complex(kind=dp) :: omega_list(kubo_nfreq)
    real(kind=dp)    :: omega, rfac
    complex(kind=dp) :: prod, cdum, cfac

    allocate (HH(Ndim, Ndim))
    allocate (delHH(Ndim, Ndim, 3))
    allocate (UU(Ndim, Ndim))
    allocate (D_h(Ndim, Ndim, 3))
    allocate (AA(Ndim, Ndim,3))
    omegamatrix=cmplx_0
    !omegamatrix=0d0
    lfreq = .false.
    lfermi = .false.
...
!for getting the demominator     
      call wham_get_eig_deleig_temp(kpt, eig, del_eig, HH, delHH, UU)
      call wham_get_D_h_temp(delHH, UU, eig, D_h)
     !Modifield by glp
! Here I apply a scissor operator to the conduction bands, if required in the input
    if (shc_bandshift) then
      eig(shc_bandshift_firstband:) = eig(shc_bandshift_firstband:) + shc_bandshift_energyshift
    ...
    AA=cmplx_0    
     call pw90common_fourier_R_to_k_vec_temp(kpt, AA_R, OO_true=AA)
     do i = 1, 3
       AA(:, :, i) = utility_rotate(AA(:, :, i), UU,Ndim)
     enddo
      AA = AA + cmplx_i*D_h ! Eq.(25) WYSV06
    ! AA = cmplx_i*D_h
    ! AA=2d0
   call berry_get_js_k_temp_way2(kpt, eig, del_eig(:, shc_alpha), delHH(:, :, shc_alpha), &
                 D_h(:, :, shc_alpha), UU, js_k)
! 要改回来
    ...
    if (lfreq) then
      call pw90common_get_occ_temp(eig, occ_freq, fermi_energy_list(1))
    elseif (lfermi) then
      ! get occ for different fermi_energy
      do i = 1, nfermi
   call pw90common_get_occ_temp(eig, occ_fermi(:, i), fermi_energy_list(i))
     end do
    end if
 ...
 !prod = -rfac*cmplx_i*AA(n, m, shc_alpha) * rfac*cmplx_i*AA(m, n, shc_beta)
      ...
      !这里要改回来
      !disabbled by glp,要改回来 
       prod = js_k(n, m)*cmplx_i*rfac*AA(m, n, shc_beta)
      
      !prod = js_k(n, m)*cmplx_i     
      !prod=D_h(m,n,shc_beta)
        ...
        !if ( abs(eig(m)-eig(n))<1.0e-3_dp) then
        !       cycle
        !endif


        !this is origin 
        omegamatrix(n,m) =  rfac*aimag(prod)/nslab
        !this is orginend
        


        !changed by glp
        !omegamatrix(n,m) = js_k(n,m)
        !omegamatrix(n,m) = aimag(prod)/nslab
        !changed end here

        !if ( abs(eig(m)-eig(n))<1.0e-2_dp) then
        !    cycle
        !endif
        !omegamatrix(n,m) =rfac*aimag(prod)/nslab
          ...
          !    omega = omega + aimag(prod) 
          !    omega = omega + aimag(prod)*rfac/(nslab)
          !modifield by glp      
    end if
...
! summing over the omegamatrix by the occupytion number
    do i=1,nfermi
    omega=cmplx_0
    occupynum=int(sum(occ_fermi(:,i)))
    !这是为了获得占据能带数目

    do n=1,occupynum
     do m=occupynum+1,Ndim
        omega=omega+omegamatrix(n,m)
     enddo !end n
    enddo !end m
    !这一步是做求和

    if (lfermi) then
    shc_k_fermi(i)=shc_k_fermi(i)+omega   
    endif 
    !把每条带的贡献累积起来
    enddo 
    ...
    if (lband) then
     ...
     do m=1,Ndim
       ...
       if ( abs(eig(m)-eig(n))<5.0e-3_dp) then
              cycle
       endif
     shc_k_band(n) = shc_k_band(n)+omegamatrix(n,m)
     !shc_k_band(n) = omegamatrix(n,n)
     
     !if (  abs(aimag(omegamatrix(n,n))) > 0.001  ) then
     !write(*,*) 'error, sz expectation is not real'
     !endif
  ...
  contains
    ...
    !                   PRIVATE PROCEDURES                      !
    !===========================================================!
    subroutine berry_get_js_k_temp_way2(kpt, eig, del_alpha_eig, delHH_alpha, D_alpha_h, UU, js_k)
      !====================================================================!
      !                                                                    !
      ! Contribution from point k to the
      !   <psi_k | 1/2*(sigma_gamma*v_alpha + v_alpha*sigma_gamma) | psi_k>
      ...
      use w90_constants, only: dp, cmplx_0, cmplx_i
      use w90_utility, only: utility_rotate
      use w90_parameters, only: slab_appro,num_wann,Ndim,shc_alpha, shc_gamma, shc_method
      use w90_postw90_common, only: pw90common_fourier_R_to_k_new, &
        pw90common_fourier_R_to_k_vec,pw90common_fourier_R_to_k_temp
      use w90_get_oper, only: SS_R, SR_R, SHR_R, SH_R, HH_R, SAA_R, SBB_R
      ...
      ! internal vars
      complex(kind=dp)    :: B_k(Ndim, Ndim)
      complex(kind=dp)    :: K_k(Ndim, Ndim)
      complex(kind=dp)    :: L_k(Ndim, Ndim)
      complex(kind=dp)    :: S_w(Ndim, Ndim)
      complex(kind=dp)    :: S_k(Ndim, Ndim)
      complex(kind=dp)    :: SR_w(Ndim, Ndim, 3)
      complex(kind=dp)    :: SR_alpha_k(Ndim, Ndim)
      complex(kind=dp)    :: SHR_w(Ndim, Ndim, 3)
      complex(kind=dp)    :: SHR_alpha_k(Ndim, Ndim)
      complex(kind=dp)    :: SH_w(Ndim, Ndim,3)
      complex(kind=dp)    :: SH_k(Ndim, Ndim)
      complex(kind=dp)    :: eig_mat(Ndim, Ndim)
      complex(kind=dp)    :: del_eig_mat(Ndim, Ndim)
      complex(kind=dp)    :: test(Ndim, Ndim),test1(Ndim, Ndim)
      !ryoo
      complex(kind=dp)    :: SAA(Ndim, Ndim, 3, 3)
      complex(kind=dp)    :: SBB(Ndim, Ndim, 3, 3)
      complex(kind=dp)    :: VV0(Ndim, Ndim)
      complex(kind=dp)    :: spinvel0(Ndim, Ndim)
      integer :: i, j,n
      ...
      ! QZYZ18 Eq.(36)
    ! call pw90common_fourier_R_to_k_new(kpt,SS_R(:, :, :, shc_gamma), OO=S_w)
      call pw90common_fourier_R_to_k_temp(kpt,SS_R(:, :, :, shc_gamma),S_w,0)
   do i=1,Ndim
      do j=1,Ndim
        if  ( abs(S_w(i,j)-conjg(S_w(j,i))) > 1e-3 ) then 
         write(*,*) 'hermitian is not preserved in the sk '
        endif
      enddo
   enddo
      ...
      ! QZYZ18 Eq.(30)
      S_k = utility_rotate(S_w, UU, Ndim)
      ...
      if (index(shc_method, 'qiao') > 0) then !if Qiao
        ...
        ! < u_k | sigma_gamma | \partial_alpha u_k >, QZYZ18 Eq.(26)
        ! QZYZ18 Eq.(37)
        call pw90common_fourier_R_to_k_vec_temp(kpt, SR_R(:, :, :, shc_gamma, :), OO_true=SR_w)

!        SR_w=cmplx_0
        !by glp,要改回来
        ! QZYZ18 Eq.(31)
        SR_alpha_k = -cmplx_i*utility_rotate(SR_w(:, :, shc_alpha), UU,Ndim)
        ...
        ! < u_k | sigma_gamma.H | \partial_alpha u_k >, QZYZ18 Eq.(27)
        ! QZYZ18 Eq.(38)
        call pw90common_fourier_R_to_k_vec_temp(kpt, SHR_R(:, :, :, shc_gamma, :), OO_true=SHR_w)
!        SHR_w=cmplx_0
       ! by glp,要改回来     
        ! QZYZ18 Eq.(32)
        SHR_alpha_k = -cmplx_i*utility_rotate(SHR_w(:, :, shc_alpha), UU,Ndim)
        ! QZYZ18 Eq.(39)
    SH_w=cmplx_0
...
!   call pw90common_fourier_R_to_k_vec_temp(kpt,SH_R,SH_w)
   !by glp,要改回来 
      ...
      ! QZYZ18 Eq.(32)
        SH_k = utility_rotate(SH_w(:, :, shc_gamma), UU, Ndim)
        L_k = SHR_alpha_k + matmul(SH_k, D_alpha_h)
      ...
      !                                     OO_dy=delHH(:, :, 2), &
      !                                     OO_dz=delHH(:, :, 3))
     call pw90common_fourier_R_to_k_temp(kpt,SAA_R(:, :, :, shc_gamma,shc_alpha),SAA(:, :, shc_gamma, shc_alpha),0)
     call pw90common_fourier_R_to_k_temp(kpt,SBB_R(:, :, :, shc_gamma,shc_alpha),SBB(:, :, shc_gamma, shc_alpha),0)  
     call pw90common_fourier_R_to_k_temp(kpt,HH_R,HH,0)
     call pw90common_fourier_R_to_k_temp(kpt,HH_R,delHH(:, :, 1),1)
     call pw90common_fourier_R_to_k_temp(kpt,HH_R,delHH(:, :, 2),2)
     call pw90common_fourier_R_to_k_temp(kpt,HH_R,delHH(:, :, 3),3)
! modified by glp
     
        VV0(:, :) = utility_rotate(delHH_alpha(:, :), UU, Ndim)
        SAA(:, :, shc_gamma, shc_alpha) = utility_rotate(SAA(:, :, shc_gamma, shc_alpha), UU, Ndim)
        SBB(:, :, shc_gamma, shc_alpha) = utility_rotate(SBB(:, :, shc_gamma, shc_alpha), UU, Ndim)

        spinVel0(:, :) = matmul(VV0(:, :), S_k(:, :)) + &
                         matmul(S_k(:, :), VV0(:, :))

        do n = 1, Ndim
          do m = 1, Ndim !RPS19 Eq.(21) and Eq.(26)
            js_k(n, m) = spinVel0(n, m) &
                         - cmplx_i*(eig(m)*SAA(n, m, shc_gamma, shc_alpha) - SBB(n, m, shc_gamma, shc_alpha))
            js_k(n, m) = js_k(n, m) &
                         + cmplx_i*(eig(n)*conjg(SAA(m, n, shc_gamma, shc_alpha)) - conjg(SBB(m, n, shc_gamma, shc_alpha)))
      ...
      !-------------------------------------------------------------------
    end subroutine berry_get_js_k_temp_way2
    
  end subroutine berry_get_shc_klist_temp_way2
  ...
  subroutine berry_get_shc_klist_temp_way3(kpt, shc_k_fermi_slab, shc_k_freq, shc_k_band)
    !====================================================================!
    !                                                                    !
    ! Contribution from a k-point to the spin Hall conductivity on a list
    ! of Fermi energies or a list of frequencies or a list of energy bands
...
use w90_postw90_common, only:pw90common_get_occ_temp,pw90common_get_occ,pw90common_get_occ_slab,pw90common_fourier_R_to_k_vec_slab,pw90common_kmesh_spacing,pw90common_fourier_R_to_k_vec,pw90common_fourier_R_to_k_vec_temp 
    ...
    ! args
    real(kind=dp), intent(in)  :: kpt(3)
    real(kind=dp), optional, intent(out) :: shc_k_fermi_slab(nfermi,nslab)
    complex(kind=dp), optional, intent(out) :: shc_k_freq(kubo_nfreq)
    real(kind=dp), optional, intent(out) :: shc_k_band(Ndim)
    ...
    complex(kind=dp), allocatable :: AA(:, :, :)
    complex(kind=dp)              :: js_k(Ndim,Ndim),omegamatrix(Ndim,Ndim)
    complex(kind=dp)              :: wavefun_layer(num_wann)
    ...
    integer          :: occupynum,n, m,j, i,s, ifreq,k
    real(kind=dp)    :: eig(Ndim),pro
    real(kind=dp)    :: occ_fermi(Ndim,nfermi), occ_freq(Ndim)
    complex(kind=dp) :: omega_list(kubo_nfreq)
    real(kind=dp)    :: omega, rfac
    complex(kind=dp) :: prod, cdum, cfac
    ...
    if (present(shc_k_fermi_slab)) then
      shc_k_fermi_slab = 0.0_dp
      lfermi = .true.
    endif
    if (present(shc_k_band)) then
...
! Here I apply a scissor operator to the conduction bands, if required in the input
    ...
    ! AA = cmplx_i*D_h
    ! AA=2d0
    call berry_get_js_k_temp_way3(kpt, eig, del_eig(:, shc_alpha), delHH(:, :, shc_alpha), &
                  D_h(:, :, shc_alpha), UU, js_k)
! 要改回来
     ...
     do k=1,nslab   
      !这一步是做求和        
       omega=0d0
        do n=1,occupynum
           pro=0d0
           wavefun_layer=UU((k-1)*num_wann+1:k*num_wann,n)
           do s=1,num_wann
              pro=pro+wavefun_layer(s)*conjg(wavefun_layer(s))
           enddo !s
 
           do m=occupynum+1,Ndim
              omega=omega+omegamatrix(n,m)*pro
           enddo !end m
        
        enddo !end n


        shc_k_fermi_slab(i,k)=shc_k_fermi_slab(i,k)+omega    
     enddo !k
...
!    do k=1,nslab-1
!    write(*,*)shc_k_fermi_slab(i,k)-shc_k_fermi_slab(i,k+1)
!    enddo !k
  ...
  contains
    ...
    !                   PRIVATE PROCEDURES                      !
    !===========================================================!
    subroutine berry_get_js_k_temp_way3(kpt, eig, del_alpha_eig, delHH_alpha, D_alpha_h, UU, js_k)
      !====================================================================!
      !                                                                    !
      ! Contribution from point k to the
      !   <psi_k | 1/2*(sigma_gamma*v_alpha + v_alpha*sigma_gamma) | psi_k>
...
! modified by glp
      ...
      !-------------------------------------------------------------------
    end subroutine berry_get_js_k_temp_way3
    
  end subroutine berry_get_shc_klist_temp_way3
  ...
  subroutine projectcomponent(kpt,component)
   use w90_constants, only: dp, cmplx_0, cmplx_i
   use w90_parameters, only:orbital_number,num_atoms,num_wann,nslab,Ndim       
   use w90_wan_ham, only: wham_get_D_h_temp,wham_get_D_h,wham_get_eig_deleig,wham_get_eig_deleig_temp
   use w90_postw90_common, only: pw90common_fourier_R_to_k_temp,pw90common_fourier_R_to_k_new, &
        pw90common_fourier_R_to_k_vec,pw90common_fourier_R_to_k_temp
   use w90_get_oper, only:HH_R, get_HH_R,SS_R, SR_R, SHR_R, SH_R, HH_R, SAA_R, SBB_R 
   use w90_utility, only: utility_diagonalize,utility_eigensystem_c
   
      real(kind=dp), intent(in)  :: kpt(3)
      real(kind=dp), dimension(Ndim)   :: eig
      complex(kind=dp), dimension(Ndim,Ndim) :: UU
      complex(kind=dp)  :: HH(Ndim,Ndim)
      complex(kind=dp)  :: delHH(Ndim,Ndim,3)
      real(kind=dp)    :: del_eig(Ndim,3)
      complex(kind=dp) ::  a(Ndim),b(Ndim),c(Ndim)
      real(kind=dp),intent(out) :: component(Ndim) 
      integer :: k,i,j
      component=0d0
      !call wham_get_eig_deleig_temp(kpt, eig, del_eig, HH, delHH, UU)
      !HH=matmul(conjg(transpose(UU)),UU)
      call get_HH_R
      call pw90common_fourier_R_to_k_temp(kpt, HH_R, HH, 0) 
      call utility_diagonalize(HH, Ndim, eig, UU)

      
     do i=1,int(Ndim/2)
        c=UU(1:Ndim,2*i)   !the wavefunction of  the band  for spin up
        b=UU(1:Ndim,2*i-1) ! the wavefunction of the band  for spin down  
      do j=1,nslab
        k=orbital_number+(j-1)*num_wann
        component(2*i)=component(2*i)+(abs(b(k))**2+abs(c(k))**2)/2
        component(2*i-1)=component(2*i-1)+(abs(b(k))**2+abs(c(k))**2)/2  
      enddo     
     enddo
  ...
  endsubroutine  projectcomponent




  subroutine berry_get_shc_klist_temp(kpt, shc_k_fermi, shc_k_freq, shc_k_band)
    !====================================================================!
    !                                                                    !
    ! Contribution from a k-point to the spin Hall conductivity on a list
    ! of Fermi energies or a list of frequencies or a list of energy bands
...
! Here I apply a scissor operator to the conduction bands, if required in the input
   ...
   call berry_get_js_k_temp(kpt, eig, del_eig(:, shc_alpha), delHH(:, :, shc_alpha), &
                        D_h(:, :, shc_alpha), UU, js_k)
...
!    js_k=cmplx_0
        ...
        if (lfreq) then
          do ifreq = 1, kubo_nfreq
            ...
            omega_list(ifreq) = omega_list(ifreq) + cfac*aimag(prod)
               ...
               rfac = -2.0_dp/(rfac**2 + eta_smr**2)
               omega = omega + rfac*aimag(prod)/nslab
  ...
  contains
    ...
    !                   PRIVATE PROCEDURES                      !
    !===========================================================!
    subroutine berry_get_js_k_temp(kpt, eig, del_alpha_eig, delHH_alpha, D_alpha_h, UU, js_k)
      !====================================================================!
      !                                                                    !
      ! Contribution from point k to the
      !   <psi_k | 1/2*(sigma_gamma*v_alpha + v_alpha*sigma_gamma) | psi_k>
      ...
      use w90_constants, only: dp, cmplx_0, cmplx_i
      use w90_utility, only: utility_rotate
      use w90_parameters, only: num_wann,Ndim,shc_alpha, shc_gamma, shc_method
      use w90_postw90_common, only: pw90common_fourier_R_to_k_new, &
        pw90common_fourier_R_to_k_vec,pw90common_fourier_R_to_k_temp
      use w90_get_oper, only: SS_R, SR_R, SHR_R, SH_R, HH_R, SAA_R, SBB_R
...
! modified by glp
    ...
    end subroutine berry_get_js_k_temp
    
  end subroutine berry_get_shc_klist_temp
  ...
  subroutine berry_get_shc_klist_slab(kpt, shc_k_fermi, shc_k_freq, shc_k_band)
    !====================================================================!
    !                                                                    !
    ! Contribution from a k-point to the spin Hall conductivity on a list
    ! of Fermi energies or a list of frequencies or a list of energy bands
    ...
    use w90_parameters, only: num_wann,nslab,Ndim,kubo_eigval_max, kubo_nfreq, &
      ...
      fermi_energy_list, nfermi, shc_alpha, shc_beta, shc_gamma, &
      shc_bandshift, shc_bandshift_firstband, shc_bandshift_energyshift
use w90_postw90_common, only: pw90common_get_occ,pw90common_get_occ_slab,pw90common_fourier_R_to_k_vec_slab,pw90common_kmesh_spacing,pw90common_fourier_R_to_k_vec 
    use w90_wan_ham, only: wham_get_D_h_slab, wham_get_eig_deleig_slab
    use w90_get_oper, only: AA_R
    !use w90_comms, only: my_node_id
    !!!

    ! args
    real(kind=dp), intent(in)  :: kpt(2)
    real(kind=dp), optional, intent(out) :: shc_k_fermi(nfermi)
    complex(kind=dp), optional, intent(out) :: shc_k_freq(kubo_nfreq)
    real(kind=dp), optional, intent(out) :: shc_k_band(Ndim)
    ...
    ! Adaptive smearing
    !
    real(kind=dp)    :: del_eig(Ndim,2), joint_level_spacing, &
                        eta_smr, Delta_k, vdum(2)
    ...
    allocate (HH(Ndim, Ndim))
    allocate (delHH(Ndim, Ndim, 2))
    allocate (UU(Ndim, Ndim))
    allocate (D_h(Ndim, Ndim, 2))
    allocate (AA(Ndim, Ndim, 2))
    ...
    call wham_get_eig_deleig_slab(kpt, eig, del_eig, HH, delHH, UU)
    call wham_get_D_h_slab(delHH, UU, eig, D_h)
 ...
 !   end if
    ...
    call pw90common_fourier_R_to_k_vec_slab(kpt, AA_R, OO_true_slab=AA)
    do i = 1, 2
      AA(:, :, i) = utility_rotate(AA(:, :, i), UU, Ndim)
    enddo
    AA = AA + cmplx_i*D_h ! Eq.(25) WYSV06

    call berry_get_js_k_slab(kpt, eig, del_eig(:, shc_alpha), delHH(:, :, shc_alpha), &
                        D_h(:, :, shc_alpha), UU, js_k)
    ...
    if (lfreq) then
      call pw90common_get_occ_slab(eig, occ_freq, fermi_energy_list(1))
    elseif (lfermi) then
      ! get occ for different fermi_energy
      do i = 1, nfermi
    call pw90common_get_occ_slab(eig,occ_fermi(:, i),fermi_energy_list(i))
      end do
    end if
    do n = 1, Ndim
      ! get Omega_{n,alpha beta}^{gamma}
  ...
  contains
    ...
    !                   PRIVATE PROCEDURES                      !
    !===========================================================!
    subroutine berry_get_js_k_slab(kpt, eig, del_alpha_eig, delHH_alpha, D_alpha_h, UU, js_k)
      !====================================================================!
      !                                                                    !
      ! Contribution from point k to the
      !   <psi_k | 1/2*(sigma_gamma*v_alpha + v_alpha*sigma_gamma) | psi_k>
      ...
      use w90_utility, only: utility_rotate
      use w90_parameters, only: num_wann,Ndim,shc_alpha, shc_gamma, shc_method
      use w90_postw90_common, only: pw90common_fourier_R_to_k_new_slab, &
        pw90common_fourier_R_to_k_vec_slab
      use w90_get_oper, only: SS_R, SR_R, SHR_R, SH_R, HH_R, SAA_R, SBB_R
      ...
      complex(kind=dp)    :: S_w(Ndim, Ndim)
      complex(kind=dp)    :: S_k(Ndim, Ndim)
      complex(kind=dp)    :: SR_w(Ndim, Ndim, 2)
      complex(kind=dp)    :: SR_alpha_k(Ndim, Ndim)
      complex(kind=dp)    :: SHR_w(Ndim, Ndim, 2)
      complex(kind=dp)    :: SHR_alpha_k(Ndim, Ndim)
      complex(kind=dp)    :: SH_w(Ndim, Ndim, 2)
      complex(kind=dp)    :: SH_k(Ndim, Ndim)
      complex(kind=dp)    :: eig_mat(Ndim, Ndim)
      complex(kind=dp)    :: del_eig_mat(Ndim, Ndim)
      ...
      ! < u_k | sigma_gamma | u_k >, QZYZ18 Eq.(25)
      ! QZYZ18 Eq.(36)
      call pw90common_fourier_R_to_k_new_slab(kpt, SS_R(:, :, :, shc_gamma), OO_slab=S_w)
      ! QZYZ18 Eq.(30)
      S_k = utility_rotate(S_w, UU, Ndim)
      ...
      if (index(shc_method, 'qiao') > 0) then !if Qiao
        ...
        ! < u_k | sigma_gamma | \partial_alpha u_k >, QZYZ18 Eq.(26)
        ! QZYZ18 Eq.(37)
        call pw90common_fourier_R_to_k_vec_slab(kpt, SR_R(:, :, :, shc_gamma, :), OO_true_slab=SR_w)
        ! QZYZ18 Eq.(31)
        SR_alpha_k = -cmplx_i*utility_rotate(SR_w(:, :, shc_alpha), UU,Ndim)
        K_k = SR_alpha_k + matmul(S_k, D_alpha_h)
        ...
        ! < u_k | sigma_gamma.H | \partial_alpha u_k >, QZYZ18 Eq.(27)
        ! QZYZ18 Eq.(38)
        call pw90common_fourier_R_to_k_vec_slab(kpt, SHR_R(:, :, :, shc_gamma, :), OO_true_slab=SHR_w)
        ! QZYZ18 Eq.(32)
        SHR_alpha_k = -cmplx_i*utility_rotate(SHR_w(:, :, shc_alpha), UU,Ndim)
        ! QZYZ18 Eq.(39)
        call pw90common_fourier_R_to_k_vec_slab(kpt, SH_R, OO_true_slab=SH_w)
        ! QZYZ18 Eq.(32)
        SH_k = utility_rotate(SH_w(:, :, shc_gamma), UU, Ndim)
        L_k = SHR_alpha_k + matmul(SH_k, D_alpha_h)
        ...
        !RPS19 Eqs.(37)-(40)
        call pw90common_fourier_R_to_k_new_slab(kpt, SAA_R(:, :, :, shc_gamma, shc_alpha), OO_slab=SAA(:, :, shc_gamma, shc_alpha))
        call pw90common_fourier_R_to_k_new_slab(kpt, SBB_R(:, :, :, shc_gamma, shc_alpha), OO_slab=SBB(:, :, shc_gamma, shc_alpha))

        call pw90common_fourier_R_to_k_new_slab(kpt, HH_R, OO_slab=HH, &
                                           OO_dx_slab=delHH(:, :, 1), &
                                           OO_dy_slab=delHH(:, :, 2))
    ...
    end subroutine berry_get_js_k_slab
    
  end subroutine berry_get_shc_klist_slab
  ...
  subroutine berry_print_progress(loop_k, start_k, end_k, step_k)
    ...
    logical, dimension(20) :: kmesh_processed = (/(.false., i=1,20)/)
