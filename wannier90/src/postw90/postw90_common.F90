...
!==============================================================================
  ...
  public ::pw90common_wanint_data_dist,pw90common_get_occ,pw90common_get_occ_slab
  public :: pw90common_fourier_R_to_k_temp,pw90common_fourier_R_to_k, pw90common_fourier_R_to_k_new, pw90common_fourier_R_to_k_vec,pw90common_fourier_R_to_k_slab,pw90common_fourier_R_to_k_vec_slab
  public :: pw90common_fourier_R_to_k_new_slab 
  ...
  public ::pw90common_ham_slab,pw90common_get_occ_temp,pw90common_OO_qlayer2qlayer,pw90common_qlayer2qlayer_supperlattice,pw90common_fourier_R_to_k_vec_temp
  !added by glp
...
106 call io_error('Error: Problem opening file kpoint.dat in pw90common_wanint_get_kpoint_file')
  ...
  subroutine pw90common_wanint_param_dist
    ...
    call comms_bcast(slab_appro,1) 
    call comms_bcast(slab_depend,1)
...
!    call comms_bcast(num_species,1) ! Ivo: not used in postw90, right?
    ...
    call comms_bcast(nslab, 1)
    call comms_bcast(Ndim, 1)
    call comms_bcast(ijmax,1)
    call comms_bcast(shc_alpha,1) 
    call comms_bcast(orbital_number,1) 
...
!=======================================================================
  ...
  subroutine pw90common_get_occ_temp(eig, occ, ef)
    !! Compute the electronic occupancy

    use w90_constants, only: dp !,eps7
    use w90_parameters, only: num_wann,Ndim !,smear_temp
!    use w90_constants, only    : elem_charge_SI,k_B_SI

    ! Arguments
    !
    real(kind=dp), intent(in)  :: eig(Ndim)
    !! Eigenvalues
    real(kind=dp), intent(in)  :: ef
    !! Fermi level
    real(kind=dp), intent(out) :: occ(Ndim)
    !! Occupancy of states
...
!    end if
  ...
  end subroutine pw90common_get_occ_temp
  ...
  subroutine pw90common_get_occ_slab(eig, occ, ef)
    !! Compute the electronic occupancy
...
!    end if
  ...
  end subroutine pw90common_get_occ_slab
...
!=======================================================================
  ...
  subroutine pw90common_fourier_R_to_k_temp(kpt,OO_R,OO,alpha)
    !=========================================================!
    !                                                         !
    !! For alpha=0:
    ...
    use w90_constants, only: dp, cmplx_0, cmplx_i, twopi
    use w90_parameters, only: real_lattice,Ndim,ijmax,num_kpts, kpt_latt, num_wann,use_ws_distance,nslab
...
!
    ...
    real(kind=dp)                   :: kpt(3),kpt_slab(2)
    complex(kind=dp), dimension(:, :, :), intent(in)  :: OO_R
   complex(kind=dp),dimension(-ijmax:ijmax,Ndim,Ndim) ::OO_ijready_psudo,OO_ijready
    complex(kind=dp),dimension(-ijmax:ijmax,num_wann,num_wann) :: OO_ijready_new  
   complex(kind=dp), dimension(Ndim,Ndim), intent(out)   :: OO
     integer                                         :: alpha

     integer         :: ir,k, i, j, ideg,i1,i2
     real(kind=dp)   :: translattice(3,3)
    real(kind=dp)    :: rdotk,rz,rt
    complex(kind=dp) :: phase_fac
     complex(kind=dp), dimension(Ndim,Ndim)  :: Hamk_slab,Hamk_slab_psudo
    kpt_slab(1)=kpt(1)
     kpt_slab(2)=kpt(2)
     OO = cmplx_0
     Hamk_slab=cmplx_0
     Hamk_slab_psudo=cmplx_0
     OO_ijready=cmplx_0
     OO_ijready_psudo=cmplx_0
 
!call pw90common_OO_qlayer2qlayer(kpt_slab,OO_R, OO_ijready_new,alpha)      
call pw90common_qlayer2qlayer_supperlattice(kpt_slab,OO_R,OO_ijready,alpha)

!  这段代码是为了调试
!call pw90common_ham_slab(kpt_slab,OO_R,Hamk_slab,alpha)
!Hamk_slab=Hamk_slab-OO_ijready(0,:,:)
!do i=1,Ndim
!do j=1,Ndim
!if (abs(Hamk_slab(i,j))<1d-5)  then 
!        Hamk_slab(i,j)=0
...
!write(*,*)Hamk_slab  
    ...
    OO(:,:)=OO(:,:)+phase_fac*OO_ijready(i,:,:)
    enddo 
 



  end subroutine pw90common_fourier_R_to_k_temp
  ...
  subroutine pw90common_fourier_R_to_k(kpt, OO_R, OO, alpha)
    ...
    use w90_constants, only: dp, cmplx_0, cmplx_i, twopi
 use w90_parameters,only:nslab,num_kpts,kpt_latt,num_wann,use_ws_distance,slab_appro
    use w90_ws_distance, only: irdist_ws, crdist_ws, &
      wdist_ndeg, ws_translate_dist
...
!            if (slab_appro .and. irvec(3, ir)>nslab) then              
!            rdotk = twopi*(kpt(1)*real(irdist_ws(1,ideg, i, j, ir),dp)+kpt(2)*real(irdist_ws(2,ideg, i, j, ir), dp))
!           else 
...
!      if (slab_appro .and. irvec(3, ir)>nslab) then
!              rdotk = twopi*((kpt(1)*irvec(1,ir)+kpt(2)*irvec(2,ir)))
!        else 
...
subroutine pw90common_qlayer2qlayer_supperlattice(kpt, OO_R, OO_ij_super, alpha)
             ! ---------------------------------------
             ! 这个是为了构建一个超原胞的z方向上的跃迁，
             ! 
             ! 
             !---------------------------------------
 
 use w90_constants, only: dp,cmplx_0, cmplx_i, twopi
 use w90_parameters,only: num_kpts,kpt_latt,num_wann,use_ws_distance,nslab,ijmax,Ndim,slab_appro
 use w90_ws_distance, only: irdist_ws, crdist_ws, &
      wdist_ndeg, ws_translate_dist
    ...
    implicit none
    real(kind=dp)                                      :: kpt(2)
    complex(kind=dp),dimension(-ijmax:ijmax,Ndim,Ndim) :: OO_ij_super 
    complex(kind=dp),dimension(-ijmax:ijmax,num_wann,num_wann):: OO_ij
    complex(kind=dp),dimension(:, :, :), intent(in)  :: OO_R
    integer                 :: alpha,i,j,ir,m,Rz 
    real(kind=dp)   :: modify
     
  call pw90common_OO_qlayer2qlayer(kpt, OO_R, OO_ij,alpha)
  
  OO_ij_super=cmplx_0

  if (slab_appro==.true.) then
   ...
   do ir = -ijmax,ijmax
     ...
     do i=0,nslab-1
     do j=0,nslab-1
       if (abs(Rz+i-j).le.ijmax) then
          if (abs(ir) >= 1)  then         
  OO_ij_super(ir,j*num_wann+1:(j+1)*num_wann,i*num_wann+1:(i+1)*num_wann)=OO_ij(i-j+Rz,:,:)*0      
          else 
  OO_ij_super(ir,j*num_wann+1:(j+1)*num_wann,i*num_wann+1:(i+1)*num_wann)=OO_ij(i-j+Rz,:,:)
          endif 
       endif
     enddo
     enddo
...
end subroutine  pw90common_qlayer2qlayer_supperlattice





subroutine pw90common_ham_slab(kpt,OO_R,Hamk_slab,alpha)
 use w90_constants, only: dp,cmplx_0, cmplx_i, twopi
 use w90_parameters,only: Ndim,num_kpts,kpt_latt,num_wann,use_ws_distance,nslab,ijmax
 use w90_ws_distance, only: irdist_ws, crdist_ws, &
      wdist_ndeg, ws_translate_dist
 ...
 implicit none
  ...
  real(kind=dp) :: kpt(2)
  complex(kind=dp), dimension(:, :, :), intent(in)  :: OO_R
  complex(kind=dp), dimension(-ijmax:ijmax,num_wann,num_wann) :: Hij
  complex(kind=dp), dimension(Ndim,Ndim)  :: Hamk_slab
  integer                                 :: alpha
  integer          :: ir, i1,i2, ideg,Rz
  real(kind=dp)    :: rdotk
  complex(kind=dp) :: phase_fac
     
  call pw90common_OO_qlayer2qlayer(kpt, OO_R, Hij,alpha)
...
end subroutine   pw90common_ham_slab
...
subroutine pw90common_OO_qlayer2qlayer(kpt, OO_R, OO_ij, alpha)
    !=========================================================!
    !                                                         !
    !! For alpha=0:
    !! O_ij(R) --> O_ij(k) = sum_R e^{+ik.R}*O_ij(R)
    ...
    !                                                         !
    ! gonglingpu:
    ! 获取一个算符的层间耦合，为了构建板块算符而准备。  
    !=========================================================!
 use w90_comms, only: on_root
 use w90_constants, only: dp,cmplx_0, cmplx_i, twopi
 use w90_parameters,only: wannier_centres,num_kpts,kpt_latt,num_wann,use_ws_distance,nslab,ijmax
 use w90_ws_distance, only: irdist_ws, crdist_ws, &
      wdist_ndeg, ws_translate_dist
    ...
    ! Arguments
    !
    real(kind=dp) :: kpt(2), rtest(2)
    complex(kind=dp), dimension(:, :, :), intent(in)  :: OO_R
    complex(kind=dp), dimension(-ijmax:ijmax,num_wann,num_wann) :: OO_ij,&
    OO_ij_replace 
    integer                                         :: alpha
     
    integer          :: ir, i, j, ideg,Rz,a,b,c,x,y,z
    real(kind=dp)    :: rdotk,newx,newy,newz
    complex(kind=dp) :: phase_fac
    ...
    do ir = 1, nrpts
       Rz=int(irvec(3, ir))
        ...
        ! [lp] Original code, without IJ-dependent shift:
       if (abs(Rz).le.ijmax) then
  
        rdotk = twopi*dot_product(kpt(:), irvec(1:2, ir))     


        phase_fac = cmplx(cos(rdotk), sin(rdotk), dp)/real(ndegen(ir), dp)
        if (alpha == 0) then
          OO_ij(Rz,:, :) = OO_ij(Rz,:, :)+phase_fac*OO_R(:, :, ir)
        ...
        elseif (alpha == 3) then
          ...
          stop 'wrong value of alpha in pw90common_OO_qlayer2qlayer'
        endif
...
!    do i=1,num_wann
!    do j=1,num_wann
!    a=wannier_center_order(2,i)
!    b=wannier_center_order(2,j) 
!    OO_ij_replace(k,i,j)=OO_ij(k,a,b)
!    enddo
!    enddo
!    enddo
!    enddo
...
!       c=irvec(3, ir)
!
!       call pw90common_latticetransform(a,b,c,newx,newy,newz)
!       
!       x=int(newx)
!       y=int(newy)
!       z=int(newz) 
...
!
!        !rdotk = twopi*dot_product(kpt(:), irvec(1:2, ir))    
!        rdotk = twopi*(kpt(1)*x+kpt(2)*y)
!        !change in 9.16 12:10  by glp
!
!        phase_fac = cmplx(cos(rdotk), sin(rdotk), dp)/real(ndegen(ir), dp)
!        if (alpha == 0) then
!          OO_ij(Rz,:, :) = OO_ij(Rz,:, :)+phase_fac*OO_R(:, :, ir)
...
!    enddo
 ...
 end subroutine pw90common_OO_qlayer2qlayer





subroutine pw90common_fourier_R_to_k_slab(kpt, OO_R, OO_slab, alpha)
    !=========================================================!
    !                                                         !
    !! For alpha=0:
    !! O_ij(R) --> O_ij(k) = sum_R e^{+ik.R}*O_ij(R)
 ...
 use w90_constants, only: dp,cmplx_0, cmplx_i, twopi
 use w90_parameters,only:num_kpts,kpt_latt,num_wann,use_ws_distance,nslab,Ndim,ijmax
 use w90_ws_distance, only: irdist_ws, crdist_ws, &
      wdist_ndeg, ws_translate_dist
    ...
    real(kind=dp)             :: kpt(2)
    complex(kind=dp), dimension(:, :, :), intent(in)  :: OO_R
    complex(kind=dp)   :: OOijready(-ijmax:ijmax,num_wann,num_wann)
    complex(kind=dp), dimension(Ndim,Ndim), intent(out)   :: OO_slab
    integer                                         :: alpha
     
    integer          :: ir, i, j, ideg,Rz,i1,i2
    real(kind=dp)    :: rdotk
    complex(kind=dp) :: phase_fac 

    call pw90common_OO_qlayer2qlayer(kpt,OO_R, OOijready, alpha)
    OO_slab=0.0d0
     ! i1 column index
              do i1=1,nslab
                  ! i2 row index
                   do i2=1,nslab
                     if (abs(i2-i1).le.ijmax)then
  OO_slab((i2-1)*Num_wann+1:(i2-1)*Num_wann+Num_wann,(i1-1)*Num_wann+1:(i1-1)*Num_wann+Num_wann )= OOijready(i1-i2,1:Num_wann,1:Num_wann)
                     endif
                   enddo ! i2
              enddo ! i1
      end subroutine pw90common_fourier_R_to_k_slab
...
! [lp] Original code, without IJ-dependent shift:
  ...
  subroutine pw90common_fourier_R_to_k_new_slab(kpt, OO_R, OO_slab, OO_dx_slab, OO_dy_slab)
    !=======================================================!
    !                                                       !
    !! For OO:
    !! $$O_{ij}(k) = \sum_R e^{+ik.R}.O_{ij}(R)$$
    ...
    use w90_constants, only: dp, cmplx_0, cmplx_i, twopi
    use w90_parameters, only: nslab,Ndim,timing_level,ijmax,num_kpts, kpt_latt, num_wann, use_ws_distance
    use w90_ws_distance, only: irdist_ws, crdist_ws, wdist_ndeg, ws_translate_dist
    ...
    real(kind=dp)                                             :: kpt(2)
    complex(kind=dp), dimension(:, :, :), intent(in)            :: OO_R
    complex(kind=dp), optional, dimension(:, :), intent(out)   :: OO_slab
    complex(kind=dp), optional, dimension(:, :), intent(out)   :: OO_dx_slab
    complex(kind=dp), optional, dimension(:, :), intent(out)   :: OO_dy_slab
    complex(kind=dp),allocatable  :: OOij_dx(:,:,:)
    complex(kind=dp),allocatable  :: OOij_dy(:,:,:)
    complex(kind=dp),allocatable  :: OOij(:,:,:)
    integer          :: ir, i, j, ideg,i1,i2
    real(kind=dp)    :: rdotk
    complex(kind=dp) :: phase_fac

    allocate(OOij_dx(-ijmax:ijmax,num_wann,num_wann),OOij_dy(-ijmax:ijmax,num_wann,num_wann),OOij(-ijmax:ijmax,num_wann,num_wann))
    call pw90common_OO_qlayer2qlayer(kpt,OO_R, OOij_dx, alpha=1)
    call pw90common_OO_qlayer2qlayer(kpt,OO_R, OOij_dy, alpha=2) 

    if (present(OO_slab)) then 
            OO_slab=cmplx_0
           do i1=1, nslab
                  ! i2 row index
             do i2=1, nslab
                if (abs(i2-i1).le.ijmax)then
    OO_slab((i2-1)*Num_wann+1:(i2-1)*Num_wann+Num_wann,(i1-1)*Num_wann+1:(i1-1)*Num_wann+Num_wann )= OOij(i1-i2,1:Num_wann,1:Num_wann)
                endif
          enddo ! i2
      enddo ! i1
     endif 

    if (present(OO_dx_slab))  then
           OO_dx_slab=cmplx_0
           do i1=1, nslab
                  ! i2 row index
             do i2=1, nslab
                if (abs(i2-i1).le.ijmax)then
    OO_dx_slab((i2-1)*Num_wann+1:(i2-1)*Num_wann+Num_wann,(i1-1)*Num_wann+1:(i1-1)*Num_wann+Num_wann )= OOij_dx(i1-i2,1:Num_wann,1:Num_wann)
                endif
          enddo ! i2
      enddo ! i1
     endif 
    ...
    if (present(OO_dy_slab)) then
           OO_dy_slab=cmplx_0
           do i1=1, nslab
                  ! i2 row index
             do i2=1, nslab
                if (abs(i2-i1).le.ijmax)then
    OO_dy_slab((i2-1)*Num_wann+1:(i2-1)*Num_wann+Num_wann,(i1-1)*Num_wann+1:(i1-1)*Num_wann+Num_wann )= OOij_dy(i1-i2,1:Num_wann,1:Num_wann)
                endif
          enddo ! i2
      enddo ! i1
      endif

  end subroutine pw90common_fourier_R_to_k_new_slab
...
! [lp] Original code, without IJ-dependent shift:
  ...
  subroutine pw90common_fourier_R_to_k_vec_slab(kpt, OO_R, OO_true_slab, OO_pseudo)
    !====================================================================!
    !                                                                    !
    ...
    use w90_constants, only: dp, cmplx_0, cmplx_i, twopi
use w90_parameters,only: ijmax,num_kpts,kpt_latt,num_wann,use_ws_distance,nslab,Ndim
    use w90_ws_distance, only: irdist_ws, crdist_ws, wdist_ndeg, ws_translate_dist
    ...
    real(kind=dp)                                  :: kpt(2)
    complex(kind=dp), dimension(:, :, :, :), intent(in)  :: OO_R
    complex(kind=dp), optional, dimension(:, :, :), intent(out)   ::OO_true_slab
    complex(kind=dp), optional, dimension(:, :, :), intent(out)   :: OO_pseudo
    complex(kind=dp),allocatable   :: OOij1(:,:,:),OOij2(:,:,:)
    integer          :: ir, i, j, ideg,i1,i2
    real(kind=dp)    :: rdotk
    complex(kind=dp) :: phase_fac
    ...
    Ndim=nslab*num_wann
    allocate(OOij1(-ijmax:ijmax,num_wann,num_wann),OOij2(-ijmax:ijmax,num_wann,num_wann))

    call pw90common_OO_qlayer2qlayer(kpt,OO_R(:,:,:,1), OOij1, alpha=1)
    call pw90common_OO_qlayer2qlayer(kpt,OO_R(:,:,:,2), OOij2, alpha=2)
           ...
           ! i1 column index
              do i1=1, nslab
                 ! i2 row index
                    do i2=1, nslab
                        if (abs(i2-i1).le.ijmax)then
            OO_true_slab((i2-1)*Num_wann+1:(i2-1)*Num_wann+Num_wann,(i1-1)*Num_wann+1:(i1-1)*Num_wann+Num_wann,1)= OOij1(i1-i2,1:Num_wann,1:Num_wann)
                        endif
                        enddo ! i2
                   enddo ! i1
              ...
              do i1=1, nslab
                 ! i2 row index
                    do i2=1, nslab
                        if (abs(i2-i1).le.ijmax)then
    OO_true_slab((i2-1)*Num_wann+1:(i2-1)*Num_wann+Num_wann,(i1-1)*Num_wann+1:(i1-1)*Num_wann+Num_wann,2)= OOij2(i1-i2,1:Num_wann,1:Num_wann)
                        endif
                        enddo ! i2
                   enddo ! i1


  end subroutine pw90common_fourier_R_to_k_vec_slab
  ...
  subroutine pw90common_fourier_R_to_k_vec_temp(kpt, OO_R, OO_true)
    !====================================================================!
    !                                                                    !
    !! For OO_true (true vector):
    !! $${\vec O}_{ij}(k) = \sum_R e^{+ik.R} {\vec O}_{ij}(R)$$
    ...
    use w90_constants, only: dp, cmplx_0, cmplx_i, twopi
    use w90_parameters, only:slab_appro,Ndim,num_kpts, kpt_latt, num_wann, use_ws_distance
    use w90_ws_distance, only: irdist_ws, crdist_ws, wdist_ndeg, ws_translate_dist
    ...
    real(kind=dp)                                     :: kpt(3)
    complex(kind=dp), dimension(:, :, :, :), intent(in)  :: OO_R
    complex(kind=dp), dimension(Ndim,Ndim,3), intent(out)   :: OO_true
   ...
   call pw90common_fourier_R_to_k_temp(kpt,OO_R(:,:,:,1),OO_true(:,:,1),0)
   call pw90common_fourier_R_to_k_temp(kpt,OO_R(:,:,:,2),OO_true(:,:,2),0)
   call pw90common_fourier_R_to_k_temp(kpt,OO_R(:,:,:,3),OO_true(:,:,3),0)
   
   !if (slab_appro) then
   !       OO_true(:,:,3)=cmplx_0
   !endif

  end subroutine pw90common_fourier_R_to_k_vec_temp
  ...
  subroutine pw90common_latticetransform(a, b, c, x, y, z)
    ...
    ! Uinv= Umatrix
     ...
     return
  end subroutine pw90common_latticetransform
 



  subroutine pw90common_transform_r(v0, x, y, z)
     !> use Umatrix to get the new representation of a vector in new basis
     !> R= a*R1+b*R2+c*R3= x*R1'+y*R2'+z*R3'
     implicit none
     ...
     return
  end subroutine pw90common_transform_r
 ...
 subroutine pw90common_inv_r(ndim,Amat)
...
!    Overwritten by the solution matrix X for dgesv, sgesv,zgesv,zgesv.
   ...
   return
   end subroutine pw90common_inv_r
