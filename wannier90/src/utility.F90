...
module w90_utility
  ...
  public :: utility_inv2
  public :: utility_det3
  public :: utility_recip_lattice,utility_recip_lattice_slab
...
subroutine utility_recip_lattice_slab(real_lat, recip_lat, volume)  !
    !==================================================================!
    !                                                                  !
    ...
    real(kind=dp)  :: zvector(3),vvector(3)
    


 !zvector(1)= real_lat(1, 2)*real_lat(2, 3) - real_lat(2, 2)*real_lat(1, 3)
 !zvector(2)= real_lat(1, 3)*real_lat(2, 1) - real_lat(2, 3)*real_lat(1, 1)
 !zvector(3)= real_lat(1, 1)*real_lat(2, 2) - real_lat(2, 1)*real_lat(1, 2)
  zvector(1)=0d0
  zvector(2)=0d0
  zvector(3)=1d0

 recip_lat(1, 1) = real_lat(2, 2)*zvector(3) - zvector(2)*real_lat(2, 3)
 recip_lat(1, 2) = real_lat(2, 3)*zvector(1) - zvector(3)*real_lat(2, 1)
 recip_lat(1, 3) = real_lat(2, 1)*zvector(2) - zvector(1)*real_lat(2, 2)
 recip_lat(2, 1) = zvector(2)*real_lat(1, 3) - real_lat(1, 2)*zvector(3)
 recip_lat(2, 2) = zvector(3)*real_lat(1, 1) - real_lat(1, 3)*zvector(1)
 recip_lat(2, 3) = zvector(1)*real_lat(1, 2) - real_lat(1, 1)*zvector(2)


 recip_lat(3, 1) = zvector(1)
 recip_lat(3, 2) = zvector(2)
 recip_lat(3, 3) = zvector(3)
...
!   volume=zvector(1)**2+zvector(2)**2+zvector(3)**2
!   volume=sqrt(volume)    
  
  vvector(1) = real_lat(1, 2)*real_lat(2, 3) - real_lat(2, 2)*real_lat(1, 3)
  vvector(2) = real_lat(1, 3)*real_lat(2, 1) - real_lat(2, 3)*real_lat(1, 1)
  vvector(3) = real_lat(1, 1)*real_lat(2, 2) - real_lat(2, 1)*real_lat(1, 2)


   volume=vvector(1)**2+vvector(2)**2+vvector(3)**2
   volume=sqrt(volume)    
  ...
  end subroutine utility_recip_lattice_slab
...
subroutine utility_eigensystem_c(JOBZ,UPLO,N,A,W)
...
!          If INFO = 0, the eigenvalues in ascending order.
     ...
     return
  end subroutine utility_eigensystem_c
