...
contains
  ...
  subroutine hamiltonian_setup()
    ...
    use w90_parameters, only: num_wann, translate_home_cell,num_kpts, bands_plot, transport, &
    ...
    if (translate_home_cell ) use_translation=.true.  !! H.J. Zhang 2016/07/02
...
!~    end if
    ...
    have_translated = .false.
     ...
     lexist=.FALSE.
     inquire(file='wann_shift_vec.dat',exist=lexist)
       if(lexist)then
         write(stdout,*)'find wann_shift_vec.dat file!'
            open(111,file='wann_shift_vec.dat')
              read(111,*)n_shift_vec
   if(n_shift_vec.ne.num_wann)stop'wrong number of shift vectors! stop!    '
              do i=1,num_wann
                  read(111,*)ii,shift_vec(:,i)
                  end do
              close(111)
       else
         write(stdout,*)'not find wann_shift_vec.dat file!'      
...
!~      end if
      ...
      if (on_root) then
        write(stdout,*)'have used the internal translate'
...
888   format(2x, 'WF centre ', i5, 2x, '(', f10.6, ',', f10.6, ',', f10.6, ' )')
  ...
  subroutine hamiltonian_write_hr()
    ...
    use w90_io, only: io_error, io_stopwatch,stdout, io_file_unit, &
      seedname, io_date
