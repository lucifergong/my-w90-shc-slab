...
contains
  ...
  subroutine wann_main
    ...
    use w90_io, only: stdout, io_error, io_wallclocktime, io_stopwatch &
      , io_file_unit
    use w90_parameters, only: translate_home_cell,num_wann, num_cg_steps, num_iter, nnlist, &
      ...
      lenconfac, proj_site,recip_lattice, real_lattice, write_r2mn, guiding_centres, &
    ...
    use w90_utility, only: utility_translate_home,utility_frac_to_cart, utility_zgemm
...
!       end if
    ...
    if (translate_home_cell) then
      do iw = 1, num_wann
     call utility_translate_home(rave(:, iw), real_lattice, recip_lattice)
      enddo
    endif
